...
angular.module('insectIdentifierFilters', []).filter('searchFilter', function() {
	return function(items, primary, category, secondary, legs, filterClicked) {
		...
		if (aPrimary === undefined)
			aPrimary = '';
		if (aSecondary === undefined)
			aSecondary = '';
		...
		if (items[0]) {
			console.log('items[i].primaryColor: '+items[0].primaryColor+' primary: '+aPrimary);
		}
		...
		for (var i = 0; i < items.length; i++) {
			...
			if (aPrimary != '' && (items[i] !== undefined && items[i].primaryColor != primary)) {
				filteredInd[i] = true;
		...
		for (var i = 0; i < items.length; i++) {
			//console.log('tmp: '+tmp);
			if (aSecondary != '' && (items[i] !== undefined && items[i].secondaryColor != secondary)) {
				filteredInd[i] = true;
		...
		for (var i = 0; i < items.length; i++) {
			...
			if (aCategory != '' && items[i] !== undefined && (items[i].category != category)) {
				filteredInd[i] = true;
		...
		console.log('returning items: '+filtered);
/*
