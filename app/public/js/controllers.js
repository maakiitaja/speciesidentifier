...
insectIdentifierControllers.controller('LoggingCtrl', ['$scope', 'Search', '$location', '$http', 
...
insectIdentifierControllers.controller('LoggingFailureCtrl', ['$scope', 'Search', '$location', '$http', 
	function ($scope, Search, $location, $http) {
...
insectIdentifierControllers.controller('BrowseObservationsCtrl', ['$scope', 'Search', '$location', '$http', '$localStorage', 
	function ($scope, Search, $location, $http) {
		...
		$scope.toggleInsectSearch = function(visibility) {
			...
			console.log('search calendar close button: '+document.getElementById('cal_close'));
			if (visibility == 0) {
		...
		$scope.showByType = function(visibility) {
			console.log('cal_close_startDate: '+document.getElementById('cal_close_startDate'));
			console.log('cal_close_endDate: '+document.getElementById('cal_close_endDate'));
		...
		$scope.searchObservations = function(query) {
			...
			$http({
				...
				method: "GET",
				params: {location: query.location, country: query.country, place: query.place, startDate: startDate,
					endDate: endDate, name: name, language: language, category: category,
					nofarm: nofarm, organicFarm: organicFarm, nonOrganicFarm: nonOrganicFarm,
					byType: byType }
				...
				for (var i = 0; i < data.length; i++) {
					...
					var formattedDate = ds_format_date(rawDate.getDate(), rawDate.getMonth()+1, rawDate.getFullYear());	
...
insectIdentifierControllers.controller('InsectDetailCtrl', ['$scope', 'Search', '$location', '$http', 
	...
	if ($scope.prevUrl == '#/list') {
		...
		$http({url: '/collection/searchItem', params: {insectId: $scope.insect._id}, method: "GET"}).then(function(response) { 
	...
	$scope.removeFromCollection = function(insect) {
		$http({
			...
			if (data == 'success') {
				...
				$scope.messageRemovedSuccess = $scope.translations.REMOVEDINSECTFROMCOLLECTION;
				// localstorage.update
				var category = $scope.insect.category;	
				console.log('category: '+category+', with length: '+$localStorage.collection[category].length+', $localStorage.collection[category][0]: '+$localStorage.collection[category][0]);
				console.log('$scope.insect._id: '+$scope.insect._id);
				console.log('$scope.insect._id: '+$scope.insect._id);
				for (var i = 0; i < $localStorage.collection[category].length; i++) {
					console.log('$localStorage.collection[category][i]._id: '+$localStorage.collection[category][0]._id);
					if ($localStorage.collection[category][i]._id == $scope.insect._id) {
						...
						console.log('$localStorage.collection[category].length: '+ 								$localStorage.collection[category].length);
						break;
				...
				$scope.messageRemovedFailure = $scope.translations.FAILEDTOREMOVEINSECTFROMCOLLECTION;
				$scope.inCollection = 0;
	...
	$scope.add_to_collection=function() {
		...
		if (!duplicate) {
			...
			$scope.messageCollectionAddFailure = $scope.translations.ITEMALREADYINCOLLECTION;
	...
	$scope.new_search=function() {
		$scope.location.path("search").search({"returningFromDetailPage": "1", insect: $scope.insect});	
	};		
  }]);

insectIdentifierControllers.controller('AddObservationsCtrl', ['$scope', 'Search', '$location', '$http', '$localStorage', 'SearchService',
	function($scope, Search, $location, $http, $localStorage, SearchService) {
		...
		// Pagination
	 	$scope.itemsPerPage = 8;
   		$scope.currentPage = 0;
		$scope.pagedInsects = [];
		$scope.selectExistingObservation = '0'; /* mode of choosing an observation place */
		...
		$scope.addObservation = function(query) {
			...
			$http({	url: '/observation/add', 
				...
				$scope.addObservationSuccess = $scope.translations.ADDOBSERVATIONSUCCESS;
...
insectIdentifierControllers.controller('SearchCtrl', ['$scope', 'Search', '$location', '$http', '$localStorage', 'SearchService', '$timeout',
	  function($scope, Search, $location, $http, $localStorage, SearchService, $timeout ) {
		...
		if ($location.search().returningFromDetailPage == '1') {
	  		...
	  		$scope.imgs = imgs;
	  		$scope.mainImageUrl=insects[0].images[0];
	  	}
